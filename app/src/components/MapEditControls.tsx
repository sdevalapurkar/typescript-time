...
const MapEditControls = (props) => {
  ...
  */
  const updateGeosBasedOnLayers = (container: any) => {
    const numberOfLayers = container.getLayers().length - 1;

    const layer = container.getLayers()[numberOfLayers];
    const layerGeoJSON: Feature = layer._mRadius
      ? { ...layer.toGeoJSON(), properties: { ...layer.toGeoJSON().properties, radius: layer.getRadius() } }
      : layer.toGeoJSON();

    props.setGeometry(layerGeoJSON);
  };
  ...
  */
  ...
  const drawGeometries = (geometry: Feature) => {
    const container = context.layerContainer || context.map;
